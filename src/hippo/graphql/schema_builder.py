...
type model in :mod:`hippo.core.schema_typing` (issue #47), the same
...
# ISO-8601 passthrough scalars. Hippo stores temporal values as ISO
...
class GraphQLTypeBuilder:
    ...
    def input_to_dict(self, class_name: str, data: Any, mode: str) -> dict[str, Any]:
        """Generated input instance → plain dict for ``HippoClient`` writes.
    ...
    def _build_one_input(
        ...
        # because the dict handed to HippoClient keys on slot names.
        ordered = sorted(
            entity.slots,
