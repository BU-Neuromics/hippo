"""JSONLoader: load JSON array files into Hippo, with optional JSONPath support."""
...
from hippo.core.loaders.base import ConfigurableLoader, RawRecord


class JSONLoader(ConfigurableLoader):
    """Load JSON array records into Hippo.

    By default, the source file must contain a JSON array at the top level.
    Optional JSONPath support (requires ``hippo[loaders-json]``) allows
    ...
    Config keys (in addition to ConfigurableLoader keys):
        ...
        jsonpath (str): JSONPath expression to extract records from nested JSON.
                        Requires ``jsonpath-ng`` (hippo[loaders-json]).
        ...
        if self._jsonpath:
            try:
                ...
                raise ImportError(
                    ...
                    "Install with: pip install 'hippo[loaders-json]'"
                ) from exc
