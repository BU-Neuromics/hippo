"""SQLLoader: load records from a SQL query into Hippo (read-only, safe queries only)."""
...
class SQLLoader(ConfigurableLoader):
    """Load records from a SQL database into Hippo.

    Requires ``hippo[loaders-sql]`` (SQLAlchemy>=2.0).
    ...
    def fetch(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ImportError(
                "SQLLoader requires sqlalchemy. Install with: pip install 'hippo[loaders-sql]'"
            ) from exc
