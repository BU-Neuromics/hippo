...
class EntityLoader(ABC):
    ...
    def transform(self, record: RawRecord) -> TransformedRecord:
        """Map a source record to the Hippo entity schema.

        Args:
            record: Raw record from fetch().

        Returns:
            TransformedRecord ready for upsert into Hippo.
        """
        ...

    def validate(self, record: TransformedRecord, client: Any) -> list[str]:
        """Optional cross-record validation against live Hippo state.

        Args:
            record: Transformed record about to be written.
            client: Live HippoClient (for lookups).
...
class ConfigurableLoader(EntityLoader):
    ...
    Config keys:
        entity_type (str): The Hippo entity type to write to.
        external_id_field (str): Source field that holds the external ID.
        field_map (dict[str, str]): Rename source keys to Hippo field names.
