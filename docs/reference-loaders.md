...
Only exact-match pins (`==`) are supported in v1, and the pin's right-hand side is matched against the **installed pip package version** (`importlib.metadata.version()`), not a data-version slug like `mouse-115` — pinning a data-version slug fails with `HIPPO_REQUIRES_UNSATISFIED` even when that data version is installed. Matching pins against the data-version slug in `hippo_meta.reference_versions` is a planned v2 surface. If you need a minimum version, pin the lowest acceptable release and upgrade explicitly with `hippo reference upgrade`.
