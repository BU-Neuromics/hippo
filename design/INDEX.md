...
| GraphQL reference emission: raw scalar + resolved edge (dual) vs. edge-only | sec3, sec4 §4.7 | High | **🟢 Accepted 2026-07-17 ([ADR-0005](./decisions/ADR-0005-graphql-reference-emission-edge-only.md); [issue #131](https://github.com/BU-Neuromics/mosaic/issues/131)). Implementation pending.** A reference slot emits both a type-erased raw `*_id` scalar and a resolved edge (`_slot_spec`); the scalar drops the target class (misleads generic consumers like Aperture) and hard-codes a storage detail (would leak a future int surrogate key). Decision: **edge-only** — emit only the resolved relationship, remove the raw scalar, and adopt the *logical-identity boundary* invariant (physical/surrogate keys never cross the API). Clean break; references always resolve (collisions are build-time errors); multivalued cardinality is resolve-to-count for now ([#132](https://github.com/BU-Neuromics/mosaic/issues/132) tracks the cheap-count optimization). Breaking change — sequenced ahead of the Aperture reference-rendering fixes. |
